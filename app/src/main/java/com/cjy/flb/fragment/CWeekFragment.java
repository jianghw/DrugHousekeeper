 ...
 */
public class CWeekFragment extends BaseFragment {
    @Bind(R.id.listView)
    ListView mListView;
    @Bind(R.id.gridView)
    GridView mGridView;
    ...
    private void noMedicineTime(Date date, HashMap<Integer, Medicine> hashMap, long time, long nTime) throws Exception
    {
        if (time >= nTime) {
            SimpleDateFormat sdfWeek = new SimpleDateFormat("E");
            String week2 = getDatabaseWeekId(sdfWeek.format(date));
            QueryBuilder<Box> builder = DaoHolder.getBoxDao(BoxDao.Properties.Day_in_week.eq(week2));
            if (builder.buildCount().count() > 0) {
                List<Box> boxs = builder.build().forCurrentThread().list();
                for (Box box : boxs) {
     ...
     * @param hashMap
     */
    private void setMedicineTime(List<Eat> eatList, HashMap<Integer, Medicine> hashMap) throws Exception
    {
        if (eatList.size() > 0) {
            for (Eat eat : eatList) {

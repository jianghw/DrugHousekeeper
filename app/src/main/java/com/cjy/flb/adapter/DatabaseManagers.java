...
public class DatabaseManagers {
	...
	private static DatabaseManagers mDbManager;// 数据库管理器实例
	...
	public static synchronized void initializeInstance(DatabaseHelper helper) {// synchronized表示给这个方法加锁
		if (mDbManager == null) {
			mDbManager = new DatabaseManagers();
			mDbHelper = helper;
		}
	}
	 ...
	 * @return
	 */
	public static synchronized DatabaseManagers getInstance(DatabaseHelper helper) {
		if (mDbManager == null) {
